...
def check_user_state(message, state):
    return message.chat.id in user_context_state and user_context_state[message.chat.id] == state
...
bot.message_handler(func=lambda message: check_user_state(message, StateBot.Registration))
...
bot.message_handler(func=lambda message: check_user_state(message, StateBot.PhonePriceSearch))
(find_phone_price_handler)

# Enum State Bot: SupportChat = 5
# Обработчик сообщений
bot.message_handler(func=lambda message: check_user_state(message, StateBot.SupportChat))
